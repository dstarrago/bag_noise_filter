 ...
 * @author Danel
 */
public class NNInstanceNoiseMultiFilter extends AbstractInstanceNoiseFilter {
...
//  public static final int DEFAULT_VOTE_SCHEME = VOTE_SCHEME_UNANIMITY;
  ...
  public static final int DEFAULT_FILTERS_AGREEMENT = VOTE_SCHEME_MINIMUM;
  ...
  private int filtersAgreement;
  private int numNeighbors; 
  private int voteThreshold;
  private int filterAgreedThreshold;
   ...
   * 
   */
  protected Class<? extends KeelNoiseFilter>[] siFilterClasses;
  
  public NNInstanceNoiseMultiFilter(Instances dataset, 
          Class<? extends KeelNoiseFilter> siFilterClasses[], int voteScheme, 
          int numNeighbors, int filtersAgreement) {
    super(dataset, null);
    this.voteScheme = voteScheme;
    this.numNeighbors = numNeighbors;
    this.siFilterClasses = siFilterClasses;
    this.filtersAgreement = filtersAgreement;
    updateVoteThreshold();
    updateFilterAgreementThreshold();
  }

  public NNInstanceNoiseMultiFilter(Instances dataset, Class<? extends KeelNoiseFilter>... siFilterClasses) {
    this(dataset, siFilterClasses, DEFAULT_VOTE_SCHEME, DEFAULT_NEIGHBORS_NUMBER, DEFAULT_FILTERS_AGREEMENT);
  }
  ...
  private void updateFilterAgreementThreshold() {
    switch (filtersAgreement) {
      case VOTE_SCHEME_MINIMUM: filterAgreedThreshold = 1; break;
      case VOTE_SCHEME_MAJORITY: filterAgreedThreshold = numNeighbors / 2 + 1; break;
      case VOTE_SCHEME_UNANIMITY: filterAgreedThreshold = numNeighbors; break;
    }
  }
  ...
  private boolean agreedForNoise(KeelNoiseFilter[] siFilters, int index) {
    int voteCounts = 0;
    for (int i = 0; i < siFilterClasses.length; i++) {
      if (siFilters[i].isNoisy(index)) {
        voteCounts++;
      }
    }
    return voteCounts >= filterAgreedThreshold;
  }
  ...
  private void filterA() {
    for (int i = 0; i < classA.size(); i++) {
      ...
      for (int j = 0; j < nearestNeighbors.size(); j++) {
        ...
        KeelNoiseFilter[] siFilters = new KeelNoiseFilter[siFilterClasses.length];
        for (int n = 0; n < siFilterClasses.length; n++) {
          siFilters[n] = getSingleInstanceFilter(siFilterClasses[n], DS);
        }
        for (int k = 0; k < siX.size(); k++) {
          if (agreedForNoise(siFilters, k)) {
  ...
  private Instances toSimpleInstance(Instance bag) {
    ...
    try {
      ...
      Logger.getLogger(NNInstanceNoiseMultiFilter.class.getName()).log(Level.SEVERE, null, ex);
    }
