  	...
  	private void getTrainTest(){
  		...
  		for(int par = 0 ; par < numPartitions ; ++par){
  			...
  			// count the number of instances in train number par
  	  		...
  	  		for(int i = 0 ; i < numPartitions ; ++i){
  	  			if(i != par){
  	  				for(int jo = 0 ; jo < partitions[i].size() ; ++jo){
  	  					if(IterativePartitioningFilter.cleanExENN[(Integer)partitions[i].get(jo)])
  	  						tam++;
  	  					}
  	  			}
  	  		}
  	  		...
  	  		for(int i = 0 ; i < numPartitions ; ++i)
  	  			if(i != par){
  	  				for(int j = 0 ; j < partitions[i].size() ; ++j){
  	  					...
  	  					//MODIFICACION ENN!!!---------------------
  	  					if(IterativePartitioningFilter.cleanExENN[(Integer)partitions[i].get(j)]){
...
//*******************************************************************************************************************************
	 ...
	 * </p>
	 */
  	public void createPartitionFiles(String timeStamp){
        ...
        for (int i = 0; i < numPartitions ; i++) {
            ...
            String trainFN = String.format("%strain-%s-%d.dat", tempFolder, timeStamp, i + 1);
            String testFN = String.format("%stest-%s-%d.dat", tempFolder, timeStamp, i + 1);
            Files.writeFile(trainFN, outputTrain);
            Files.writeFile(testFN, outputTest);
        }
...
//*******************************************************************************************************************************
	 ...
	 * </p>
	 */	
  public void deletePartitionFiles(String timeStamp){
    for(int i = 0 ; i < Parameters.numPartitions ; ++i){
      String trainFN = String.format("%strain-%s-%d.dat", tempFolder, timeStamp, i + 1);
      String testFN = String.format("%stest-%s-%d.dat", tempFolder, timeStamp, i + 1);
      File fichero = new File(trainFN);
      fichero.delete();
      fichero = new File(testFN);
      fichero.delete();
    }
  }
