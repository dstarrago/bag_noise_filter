...
package keel.Algorithms.Preprocess.NoiseFilters.RNG;
...
public class RNG extends Metodo {
 ...
 /*Own parameters of the algorithm*/
  ...
  public Vector ejecutar (boolean[] cleanEx) {
    ...
    clasesRuido = new int[datosTrain.length];
    for (i=0; i<datosTrain.length; i++)
    	clasesRuido[i] = -1;
    ...
    //modificacion de ENN---------------------------------------------------------------
    for(int ex_ = 0 ; ex_ < cleanEx.length ; ex_++){
    	if(!cleanEx[ex_]){
    		for(int row_ = 0 ; row_ < datosTrain.length; row_++){
    			grafo[row_][ex_] = false;
    		}
    	}
    }
    //-----------------------------------------------------------------------------------
    ...
    // bieeeeen-----------------------------------------
    /*Vector ru_v = new Vector();
    Vector cl_v = new Vector();
    for (i=0, l=0; i<datosTrain.length; i++) {
        if (!marcas[i]) { //the instance will be copied to the solution
        	ru_v.add(i);
        	cl_v.add(clasesRuido[i]);
        }
    }
    
    int[] res_ru = new int[ru_v.size()];
    int[] res_cl = new int[cl_v.size()];
    for(int va = 0 ; va < res_ru.length ; ++va)
    	res_ru[va] = (Integer) ru_v.get(va);
    for(int va = 0 ; va < res_cl.length ; ++va)
    	res_cl[va] = (Integer) cl_v.get(va);*/
    
    // invertir marcas, para decir las que SI tienen ruido
    for (i=0 ; i<datosTrain.length ; i++)
    	marcas[i] = !marcas[i];
    ...
    Vector final_result = new Vector();
    final_result.add(marcas);
    final_result.add(clasesRuido);
    final_result.add(numnoise);
